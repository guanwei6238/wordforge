...
/// ## 它會隨資料變慢，而且是兩個維度
///
/// 要排出「誰的句子最少」就得掃過整個牌組，沒有早停的餘地。實測：
///
/// ```text
/// 牌組       251 →  1000 → 5107 張複習卡     11 → 26 → 80 ms
/// 句子索引    77 → 19.7 萬列（同樣牌組）      19 → 78 ms
/// ```
///
/// 現在可以接受，因為呼叫它的時候使用者正在等模型寫文章（30~90 秒），
/// 80 ms 佔 0.1%。**但它不能搬到互動路徑上。**
///
/// 真的變痛的時候，修法是取樣而不是加索引：這個清單的用途只是「可用池
/// 有 20% 的名額優先給練得少的字」，本來就不需要全牌組最佳解，
/// 而且同組還要隨機。先隨機抽幾百張卡再從中挑，就變成常數時間。
