    ...
    prompt.push_str(&format!(
        "# 硬性要求\n\
         ...
         3. 唯一允許出現的新詞是下列白名單：{targets}\n\
            **盡量多用幾個**——清單給的比需要的多，是讓你有得挑，不是讓你只用兩三個。\n\
            用到的每個新詞請自然地出現 {repeats} 次以上，並讓上下文足以推敲其意思。\n\
            用幾個由第 2 點的覆蓋率決定：新詞太少這篇就沒東西可學，\n\
            太多他會讀不下去。\n\
            **但最高原則是用法正確、句子自然**——這一條贏過「多用幾個」：\n\
            不確定某個字在這個情境的搭配詞或介系詞，就換一個字；\n\
            放進去會讓句子變奇怪、需要硬轉話題，也換一個；\n\
            粗俗冒犯不適合教材的，跳過。\n\
            教錯一個用法比少教一個字糟得多——他會把錯的記起來。\n\
        ...
        "# 學習者程度\n\
         ...
         3. 從下面這些字裡**挑 {n} 個**（至少 {min} 個），各在文章裡用剛好一次，\n\
            ...
            **不要跳號、不要重複，也不要出現沒有對應題目的空格。**\n\
            清單給的比要挖的多，是讓你有得挑：**盡量挑得到 {n} 個**，\n\
            但**用法正確、句子自然優先**——某個字硬要放進這篇會讓句子變奇怪、\n\
            或者你不確定它在這裡的搭配，就換清單裡的另一個字。\n\
            教錯一個用法比少挖一格糟得多。\n\
    ...
    let word_rule = if words.is_empty() {
        ...
        format!(
             ...
             **出題之後系統會實際檢查**那個字（或它的變化形）有沒有出現在{target}那一句裡，\n\
             ...
             # 用字取捨\n\
             清單給的比要用的多，是讓你有得挑。**最高原則是用法正確、句子自然**：\n\
             - 不確定這個字在這個情境的搭配詞或介系詞，就換清單裡的另一個字。\n\
             - 一個字硬要放進去會讓句子變得奇怪、或需要硬轉話題，也換一個。\n\
             教錯一個用法比少練一個字糟得多——他會把錯的記起來。\n\
             {usable_note}\n",
            count = count,
            native = native_lang,
            target = target_lang,
...
mod tests {
    ...
    fn review_words_are_asked_for_separately_from_new_words() {
        ...
        // 這裡原本要求「白名單盡量全部用到」，理由是「不然模型會挑著用」。
        // 現在**刻意讓它挑**：候選給得比需要的多，全部用上等於逼它把每個字
        // 都塞進去，而有些字在那個情境下就是寫不自然。
        //
        // 但「挑」不等於「能少用就少用」——少用一個字就是少學一個字。
        // 所以措辭是「盡量多用，正確性優先」，兩件事都要講到。
        assert!(
            text.contains("盡量多用幾個"),
            "少用一個字就是少學一個字，不能只講「可以不用」：{text}"
        );
        assert!(
            text.contains("最高原則是用法正確"),
            "取捨的判準要講明白：正確性贏過用字數量：{text}"
        );
        assert!(
            text.contains("教錯一個用法比少教一個字糟得多"),
            "要說得出為什麼——他會把錯的記起來：{text}"
        );
        assert!(
            text.contains("粗俗"),
