...
| 文法選單只列 `state != null` 的點 | 自己加的點練過才選得到，而沒選過就永遠練不到 |
...
**會隨資料長大的清單一定要分頁。** 判斷方式很簡單：這個清單的長度
是不是由資料庫決定的？是的話，今天看起來剛好的畫面，用三個月之後
就是捲不完的一片。查詢層要收 `limit` 與 `offset`，而且**要一起回傳
總數**——少了它，UI 只知道「這頁有幾筆」，說不出「第 2 / 7 頁」，
使用者也不知道還有沒有下一頁。

已經這樣做的地方：練習紀錄（一頁十份）、字典搜尋（`limit`）、
「你做過的句子」（一頁三句，常練的字會累積到十幾句）。

**沒有分頁就別讓它出現在共用元件裡。** 那一份「你做過的句子」掛在
`WordSenses` 上，而那個元件同時服務字典頁與複習頁——一個沒有上限的
清單會同時撐爆兩個畫面。

**只查一個布林值就不要撈統計。** 開機畫面要問的是「字典是不是空的」，
它原本呼叫整份 `dictionary_stats`，而那裡面對 285 萬列的 `sense` 做
`COUNT(DISTINCT lemma_id)`——實測 1.3 秒（冷快取更久），而複習頁在
那之前是一片空白。改成 `SELECT EXISTS(...)` 之後是 0.0 ms。
...
**「請用這些字」也要驗。** 翻譯題的 prompt 一直有列出這次要練的字，
但列出來只是請求：模型常常寫出一句通順、自然、跟那個字沒有關係的句子。
畫面上看不出異狀——題目是好題目，只是那個字沒練到，而系統還把它記成
「這次練了它」。現在指派是一題一個字，收到回應後用 `lemmas::forms`
拿整個詞族去比對目標語言那一句（`ran` 算練到 `run`），對不上就帶著
那句話回問一次。文法練習指定了點也一樣：每一題的 `grammar_point`
都要是那個點。
